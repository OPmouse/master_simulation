...
double _randG(double myu,double var,int *idum)
{
  ...
  y = (double)ran1(idum);
  noise = sqrt(-2.0*log(x))*cos(2.0*M_PI*y);
  return (myu + var*noise);
}
...
//data generation
void _DataGeneration(double *data,double myu,double var,int start,int end,int *idum,int mode){
  unsigned int i;
  if(mode==MODE_EXP){
    for (i=start; i<end; i++) {
...
int main(int argc,char *argv[]) {
  ...
  for (sn=0; sn<SNR_STEP;sn++) {
    ...
    for (iter=0;iter<iteration;iter++){
      ...
      _DataGeneration(y, 0.0,sigma_2, 0, transition_point1, &p,MODE_NORM);
	...
	_DataGeneration(y, P[sn], sigma_2, transition_point1, transition_point2, &p,MODE_NORM);
      ...
      if(MODE==MODE_EXP){
	_DataGeneration(y, P[sn],sigma_2, transition_point1, transition_point2, &p,MODE_EXP);
      }
      //N(0,sigma_2) range:[transition_point2,FFTPOINT]
      _DataGeneration(y, 0.0,sigma_2, transition_point2, FFTPOINT, &p,MODE_NORM);
	...
	for (i=0; i<FFTPOINT; i++) {
      ...
      if(Power!=0.0) {
	diff += (double)P[sn]/(double)Power;
      }
      if(Power_transition_cusum!=0) {
	diff_transition_cusum += (double)P[sn]/(double)Power_transition_cusum;
      }
