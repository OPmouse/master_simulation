...
//normal distribution
double _randG(double myu,double var,int *idum)
{
    double x,y;
    double noise=0.0;
    //nvar = 1.0;
...
double _rand_chi( int k, int *idum){
    int i;
    double z,w=0;
    
    for(i=0;i<k;i++){
        z=sqrt( -2.0*log(ran1(idum))) * sin( 2.0*M_PI*ran1(idum));
        w+=z*z;
    }
...
//data generation
void _DataGeneration(double *data,double mean,double var,int start,int end,int *idum,int mode){
    unsigned int i;
    if(mode==MODE_EXP){
        for (i=start; i<end; i++) {
    ...
    if(mode==MODE_NORM){
        for (i=start; i<end; i++) {
            data[i] = _randG(mean,var, idum);
        }
    }
    if(mode==MODE_CHI){
        for (i=start; i<end; i++) {
            data[i] = _rand_chi(var, idum);
        }
    }
...
void _loglikelihood_OFF2ON(double *l,double *data,double P,int mode){
    ...
    for (i=0; i<FFTPOINT; i++) {
        ...
        //case 3 : chi-square distribution
        if(mode==MODE_CHI){
            l[i] = data[i]*data[i]*0.5-(P+0.5)*data[i]+0.5*P*P-0.5*log(data[i]);
        }
    }
}
...
void _loglikelihood_ON2OFF(double *l,double *data,double P,int mode){
    ...
    if(mode==MODE_CHI){
        for (i=0; i<FFTPOINT; i++) {
            l[i] = 0.5*log(data[i])-data[i]*data[i]*0.5+(P+0.5)*data[i]-0.5*P*P;
        }
    }
}
...
double _MEAN(double *data,double transition_p1,double transition_p2){
    int i;
    double sum = 0;
    double mean;
    for (i=transition_p1; i<transition_p2; i++) {
        sum += data[i];
    }
    mean =(double)sum/(double)(transition_p2-transition_p1+1);
    return mean;
}
...
int main() {
    ...
    for (sn=0; sn<SNR_STEP;sn++) {
        ...
        for (iter=0;iter<iteration;iter++){
            ...
            _DataGeneration(y, 0, sigma_2, 0, transition_point1, &p,MODE_CHI);
            
            //N(0,P+sigma_2) range:[transition_point1,transition_point2]
            if(CHANNEL==AWGN){
                _DataGeneration(y, P[sn], sigma_2, transition_point1, transition_point2, &p,MODE_NORM);
            }
            if(CHANNEL==FADING){
                _DataGeneration(y,0,P[sn], transition_point1, transition_point2, &p,MODE_EXP);
            }
            //N(0,sigma_2) range:[transition_point2,FFTPOINT]
            _DataGeneration(y, 0, sigma_2, transition_point2, FFTPOINT, &p,MODE_CHI);
            ...
            Transition_point_ON2OFF_cusum[sn] = find_id(g_cusum_ON2OFF, max(g_cusum_ON2OFF, FFTPOINT), FFTPOINT);
            Transition_point_OFF2ON_cusum[sn] = find_id(g_cusum_OFF2ON, max(g_cusum_OFF2ON, Transition_point_ON2OFF_cusum[sn]), Transition_point_ON2OFF_cusum[sn]);
            ...
            /*
            if (find_id(g_cusum_OFF2ON, SecondMax(g_cusum_OFF2ON, FFTPOINT), FFTPOINT)<Transition_point_ON2OFF_cusum[sn] || find_id(g_cusum_OFF2ON, max(g_cusum_OFF2ON, FFTPOINT), FFTPOINT)<Transition_point_ON2OFF_cusum[sn]) {
                Transition_point_OFF2ON_cusum[sn] = find_id(g_cusum_OFF2ON, max(g_cusum_OFF2ON, Transition_point_OFF2ON_cusum[sn]), Transition_point_ON2OFF_cusum[sn]);
            ...
            Power = _MEAN(y,0,FFTPOINT-1);
            //Power_transition_cusum = _MEAN(y,Transition_point_OFF2ON_cusum[sn],Transition_point_ON2OFF_cusum[sn]);
            Power_transition_cusum = _MEAN(y,Transition_point_OFF2ON_cusum[sn],Transition_point_ON2OFF_cusum[sn]);
            
            //printf("sn=%d,Power=%lf,Power_transition=%lf\n",sn,Var,Var_transition);
            diff += P[sn]/Power;
            //CUSUM
            diff_transition_cusum += P[sn]/Power_transition_cusum;
            
            Power = 0.0;
            Power_transition_cusum = 0.0;
        }
