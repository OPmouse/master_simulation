...
\begin{figure}[!htp]
\begin{center}
\includegraphics[width=120mm,clip]{fujii_sdb.eps}
...
As explained in the previous section, measurement-based spectrum database with spectrum sharing is constructed with considering received power value reported from Secondray Users at different position. However, because all received samples during the sensing period is utilized to calculate the received power value, the reported value may include noise component if a communication status transition of Primary User occurs during the sensing period.
